...
exp(predict(model.AIC, ames_test) ) %>%
  mean()
...
# Extract Predictions
predict.full1 <- exp(predict(model.full, ames_train))

# Extract Residuals
resid.full1 <- ames_test$price - predict.full1

# Calculate RMSE
rmse.full1 <- sqrt(mean(resid.full1^2))
rmse.full1
...
# Calculate RMSE
rmse.full2 <- sqrt(mean(resid.full2^2))
rmse.full2

# Extract Predictions
predict.full3 <- exp(predict(model.AIC, ames_train))

# Extract Residuals
resid.full3 <- ames_test$price - predict.full3

# Calculate RMSE
rmse.full3 <- sqrt(mean(resid.full3^2))
rmse.full3 
```
...
model.BIC <- stepAIC(model.full, k = log(nrow(ames_test)))
